...
2. Launch a Worker subagent with `isolation: "worktree"` and `model: "sonnet"`, passing the task ID and the `--root` path to the main repo. Workers execute well-specified plans — Opus is unnecessary and should stay in the interactive session for decision-making
